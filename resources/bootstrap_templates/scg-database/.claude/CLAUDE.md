...
Shared Go database library. Creates DB connections/pools; no logging; returns wrapped errors only.
...
`scg-database` provides DB connection/pool setup and small utilities.

## Hard Boundaries

- **No logging.** Ever. Return rich, wrapped errors.
- No ORM. Keep it lean and explicit.
- Single clean entrypoint: `database/database.go`.
...
- Expose a small, stable surface area.
- Avoid global state; accept context and configs.
