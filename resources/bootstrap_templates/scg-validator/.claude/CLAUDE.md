...
Shared Go validation library (Laravel-like). Rules, custom messages, aggregation; used by all services.
...
`scg-validator` is a shared Go validation library with Laravel-like behavior:
- Aggregates validation errors (not fail-fast globally)
- Supports `bail` (fail-fast per field when bail is present)
- Supports custom rule messages via configuration
- Enforces consistent rule patterns across packages

## Hard Boundaries

- No magic strings. All rule names/messages are private constants and prefixed (e.g. `integerRuleDefaultMsg`).
- Rules must use the shared config/custom-message mechanism; do not invent new ones.
- Don’t “optimize” by changing behavior. Compatibility > cleverness.
...
## Rule Refactor Pattern (Must Follow)

- One rule per file where it improves clarity.
- Shared helpers go in `utils/helpers.go`.
- Return `*contract.ValidationError` consistently.
- Respect nullable behavior and message overrides.
