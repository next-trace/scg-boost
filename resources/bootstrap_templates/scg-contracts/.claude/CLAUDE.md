...
`scg-contracts` contains shared **schemas/contracts** (OpenAPI/Proto/JSON schema/etc.) and generation scripts.

## Hard Boundaries

- Backward compatibility is king.
- Any breaking contract change requires explicit versioning.
- Generated artifacts must be reproducible; scripts must be deterministic.

## Development Commands

```bash
# If this repo is shell-driven:
