...
`scg-config` is a shared configuration library.
Typical responsibilities:
- typed config structs + defaults
- env/SSM mapping helpers (as applicable)
- validation hooks (prefer `scg-validator`)

## Hard Boundaries

- No service-specific logic.
- No secret values in code; only keys/paths.
- Prefer stable, explicit config keys (no magic).
